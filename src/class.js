...
const rosters = _ =>
  rp('https://classes.cornell.edu/api/2.0/config/rosters.json').then(body =>
    JSON.parse(body).data.rosters.map(rost => ({
...
const subjects = roster =>
  (roster
    ...
    .then(rslug =>
      rp(
        `https://classes.cornell.edu/api/2.0/config/subjects.json?roster=${rslug}`
      )
    )
    .then(body =>
      JSON.parse(body).data.subjects.map(subj => ({
        slug: subj.value,
        descr: subj.descr,
      }))
    );
...
const search = query =>
  Promise.resolve(query)
    ...
    .then(query =>
      query.subject.map(
        subject =>
          `https://classes.cornell.edu/api/2.0/search/classes.json?roster=${query.roster}&subject=${subject}&q=${query.search}`
      )
    )
