...
const notifyVersion = function() {
  if (Number(process.version.match(/^v(\d+\.\d+)/)[1]) >= 6) return;
  console.log(
    'running node version ' + process.version + '. Need at least 6.0.0.'
  );
  process.exit(1);
};

const extend_strings = function() {
  String.prototype.capitalize = function() {
    return this.charAt(0).toUpperCase() + this.slice(1);
  };
};

const filter_on = function(query) {
  return function(x) {
...
const merge_with_id = (a, b, fid) => {
  ...
  // add b in to the mix
  const b_map = b.reduce(
    (acc, x) => Object.assign(acc, { [fid(x)]: x }),
    a_map
  );
